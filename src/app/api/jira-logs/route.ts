...
function formatJiraBaseUrl(): string {
  ...
  const normalized = domain.startsWith('https://')
    ? domain
    : `https://${domain}`
...
function getAuthHeader(): string {
  ...
  if (!email || !token)
    throw new Error('JIRA_USER_EMAIL or JIRA_API_TOKEN is not set')
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const emailParam = req.nextUrl.searchParams
      .get('email')
      ?.toLowerCase()
      .trim()
    ...
    const authorClause = accountId
      ? `worklogAuthor in (${accountId})`
    ...
    await mapWithConcurrency(issueKeys, 6, async (issueKey) => {
      ...
      const wlResp = await httpClient.get<{ worklogs?: JiraApiWorklog[] }>(
        worklogUrl,
        {
          headers: {
            Authorization: authHeader,
